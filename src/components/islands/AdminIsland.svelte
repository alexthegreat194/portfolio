...
<script>
	...
	const onSubmit = (e) => {
		...
		axios
			...
			.then(() => {
				axios.post('/api/admin', { password }).then((response) => {
...
<div class="h-screen flex flex-col justify-center items-center overflow-auto">
	{#if emails.length <= 0}
		<form
			...
			class="flex flex-col gap-2 bg-gray-500 p-4 w-80 rounded-xl border-2 border-gray-800 shadow"
		>
			<label for="password" class="text-2xl font-bold">Admin Password</label>
			<input name="password" type="password" class="rounded text-xl" id="password" />
